...
export default function PromptInput({ onGenerate, isLoading, onOpenHistory }: Props) {
    ...
    <form onSubmit={handleSubmit} className="w-full">
      ...
      <div className="flex gap-3 mt-4">
        <button
          ...
          className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold py-4 px-6 rounded-xl shadow-lg hover:shadow-purple-500/25 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 transition-all duration-300 text-lg"
        >
          {isLoading ? (
        ...
        <button
          ...
          className="px-6 py-4 bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl text-white font-semibold hover:bg-white/20 transition-all duration-300"
          title="View History"
        >
          📚
        </button>
      ...
      <p className="text-center text-purple-400/60 text-xs mt-2">
        🎁 Free tier: 5 generations per day | 💾 Snippets saved locally
      </p>
    </form>
  );
}